...
#
def contact():
    mess._show(title='Contact us', message="Please contact SNGIST Institute")
...
def psw():
    ...
    if exists1:
        ...
        new_pas = tsd.askstring('Old Password not found here', 'Please enter a new password below', show='*')
        if new_pas == None:
            mess._show(title='No Password Entered', message='Password not set!! Please try again')
        else:
            tf = open("TrainingImageLabel\psd.txt", "w")
...
def TrackImages():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50) and str(df.loc[df['SERIAL NO.'] == serial]['NAME'].values)!=' ':
                ts = time.time()
...
##            self.from_entry = tk.Entry('hellooo')
##            self.from_entry.grid(row=0, column=1)
            ...
            tk.Label(master, text="Attachment:").grid(row=4)
            self.attachment_label = tk.Label(master, text="No file selected")
            self.attachment_label.grid(row=4, column=1)

            # create buttons
            tk.Button(master, text="Select attendance file", command=self.select_file).grid(row=4, column=2)
            tk.Button(master, text="Send", command=self.send_email).grid(row=5, column=1)

        def select_file(self):
            # open a file dialog to select a file
            file_path = filedialog.askopenfilename()
            self.attachment_label.config(text=file_path)

        def send_email(self):
            # get values from entries
            from_email = 'brilliantmindsprojectmail@gmail.com'
            to_email = self.to_entry.get()
            subject = "Attendance"
            message = "Sending Attendace file, PFA"
            ...
            # attach the message and attachment
            msg.attach(MIMEText(message))
            attachment_file_path = self.attachment_label.cget("text")
            if attachment_file_path:
                with open(attachment_file_path, "rb") as attachment_file:
                    part = MIMEApplication(attachment_file.read(), Name=attachment_file_path)
            ...
            with smtplib.SMTP('smtp.gmail.com', 587) as smtp:
                smtp.ehlo()
                smtp.starttls()
                smtp.login(from_email, 'thveunahhidwhgwv') # replace with your email password
                smtp.send_message(msg)

            # show a message box when email is sent
            tk.messagebox.showinfo("Email Sent", "Your email has been sent.")
...
# create the GUI window
    root = tk.Tk()
    email_sender_gui = EmailSenderGUI(root)
    root.mainloop()
...
window.title("Attendance Management System - SNGIST")
window.configure(background='#262523')

frame1 = tk.Frame(window, bg="#CAD6FF")
frame1.place(relx=0.02, rely=0.17, relwidth=0.39, relheight=0.80)

frame2 = tk.Frame(window, bg="#CAD6FF")
frame2.place(relx=0.42, rely=0.17, relwidth=0.38, relheight=0.80)

frame5 = tk.Frame(window, bg="#CAD6FF")
frame5.place(relx=0.82, rely=0.17, relwidth=0.15999, relheight=0.39)

frame6 = tk.Frame(window, bg="#CAD6FF")
frame6.place(relx=0.82, rely=0.58, relwidth=0.15999, relheight=0.39)
...
frame3 = tk.Frame(window, bg="#00c6ce")
...
tv= ttk.Treeview(frame1,height =19,columns = ('name','date','time'))
...
trackImg = tk.Button(frame5, text="Take Attendance", command=TrackImages  ,fg="black"  ,bg="#1AFD03"  ,width=15  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
trackImg.place(x=10,y=200)
trackImg2 = tk.Button(frame5, text="Face Mask Detection", command=TrackImages2  ,fg="black"  ,bg="#1AFD03"  ,width=15  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
trackImg2.place(x=10,y=100)
trackImg3 = tk.Button(frame5, text="Send Email", command=TrackImages3  ,fg="black"  ,bg="#1AFD03"  ,width=15  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
trackImg3.place(x=10,y=10)
quitWindow = tk.Button(frame6, text="Quit", command=window.destroy  ,fg="black"  ,bg="red"  ,width=15 ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
quitWindow.place(x=10,y=170)

##################### END ######################################
#root = Tk()
pathb = r'C:\Users\ayyap\Downloads\Project SNGIST\Face Recognition\Face_recognition_based_attendance_system\logo.JPG'
image1 = Image.open("logo.JPG")
image1 = image1.resize((50, 50), Image.ANTIALIAS)
image1 = ImageTk.PhotoImage(image1)
label1 = tk.Label(window,image=image1)
label1.image = image1
label1.place(x=1060,y=60)

message6 = tk.Label(window, text="SNGIST " ,fg="black",bg="white" ,width=10 ,height=2,font=('times', 15, ' bold '))
message6.place(x=1120, y=60)
